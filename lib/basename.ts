...
/**
 * Resolves a Basename to its Ethereum address using the server-side API
...
export async function resolveBasename(name: string): Promise<string | null> {
    try {
        ...
        console.log(`[Basename] Resolving via API: ${fullName}`);

        const response = await fetch(`/api/resolve-basename?name=${encodeURIComponent(fullName)}`);

        if (!response.ok) {
            const errorData = await response.json().catch(() => ({}));
            console.error('[Basename] API error:', errorData);
            return null;
        }
        ...
        if (data.address) {
            console.log(`[Basename] Resolved ${fullName} to:`, data.address);
            return data.address;
        }
